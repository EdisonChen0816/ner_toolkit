...
from src.bilstm_crf_tf import BiLstmCrf
import os
import tensorflow as tf
from gensim.models import KeyedVectors
from src.w2v_bilstm_crf_tf import W2VBiLstmCrf
from src.bert_crf_tf import BertCrf
from src.bert_bilstm_crf_tf import BertBiLstmCrf


def test_ac_match(text):
    config = loadyaml('conf/NER.yaml')
    logger = setlogger(config)
    s = ACMatch(config['entity_path'])
    for e in s.ac_match(text):
        print(e)
        logger.info(e)


def test_bilstm_crf_tf(texts):
    config = loadyaml('conf/NER.yaml')
    logger = setlogger(config)
    ...
    blc_cfg = {
        ...
        'summary_path': config['bilstm_crf_summary_path']
    }
    model = BiLstmCrf(**blc_cfg)
    model.fit()
    model.load(config['bilstm_crf_predict_path'])
...
if __name__ == '__main__':
    ...
    test_bilstm_crf_tf(['中国很大'])
    test_w2v_bilstm_crf_tf(['北京很大', '上海很大'])
    test_bert_crf_tf('招商银行田惠宇行长在股东大会上致辞')
    test_bert_bilstm_crf_tf('招商银行田惠宇行长在股东大会上致辞')