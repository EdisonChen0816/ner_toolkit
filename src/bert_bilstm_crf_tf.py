...
def create_model(bert_config, is_training, input_ids, input_mask, tag2label, num_units, labels, seq_lens):
...
def model_fn_builder(bert_config, init_checkpoint, tag2label, num_units, learning_rate=5e-5, num_train_steps=0,
                     num_warmup_steps=0):
...
class BertBiLstmCrf:
    ...
    def __init__(self, bert_path, train_path, eval_path, num_units, max_length, batch_size, save_path, learning_rate, epoch, save_checkpoints_steps, tf_config, tag2label=None, do_lower_case=True, warmup_ratio=.1):
    ...
    def save_config(self):
        ...
        self.config['num_units'] = self.num_units
